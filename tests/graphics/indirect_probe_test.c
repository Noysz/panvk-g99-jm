/* indirect_probe_test.c
 *
 * Phase 4.4 gate (T4.4.0) — empirical half of the indirect-draw survey.
 *
 * The static survey predicts two specific runtime behaviours on v9/JM. This
 * probe checks both against the hardware instead of trusting the reading.
 *
 *   direct        vkCmdDraw, control. Must render the baseline triangle.
 *   draw_indirect vkCmdDrawIndirect with a valid VkDrawIndirectCommand in a
 *                 GPU buffer. panvk_per_arch(CmdDrawIndirect) for PAN_ARCH >= 9
 *                 has an empty body (jm/panvk_vX_cmd_draw.c:3026-3031), so the
 *                 prediction is that nothing at all is drawn, with no error and
 *                 no GPU fault.
 *
 * A pass here means the prediction held. It is NOT a claim that indirect draw
 * works - the opposite.
 *
 * Env: IND_CASE=direct|draw_indirect, IND_PPM=<path>, PANVK_ICD_SO.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void){
    const char*cs=getenv("IND_CASE"); if(!cs||!cs[0]) cs="direct";
    int use_indirect = !strcmp(cs,"draw_indirect");
    printf("=== case: %s ===\n", cs);
    printf("path under test : %s\n",
           use_indirect ? "vkCmdDrawIndirect" : "vkCmdDraw (control)");
    printf("prediction      : %s\n",
           use_indirect ? "0 pixels, no error, no GPU fault (empty stub at v9)"
                        : "512/4096, centre red (baseline geometry)");
    ...
    printf("vkCmdDrawIndirect entry point : %s\n", CDrawInd ? "present" : "NULL");
    if (use_indirect && !CDrawInd) {
        printf("RESULT: entry point absent -> NOT IMPLEMENTED at the loader level\n");
        return 0;
    }
    ...
    /* indirect command buffer holding a valid VkDrawIndirectCommand */
    ...
    if(use_indirect){
        ...
        void*p=NULL; CHECK(MM(dev,imem,0,sizeof(VkDrawIndirectCommand),0,&p),"map");
        VkDrawIndirectCommand c={.vertexCount=3,.instanceCount=1,
                                 .firstVertex=0,.firstInstance=0};
        memcpy(p,&c,sizeof(c)); UM(dev,imem);
        printf("indirect command written : vertexCount=3 instanceCount=1\n");
    }
    ...
    size_t vz=0,fz=0;
    char*vc=rf(getenv("IND_VS")?getenv("IND_VS"):"triangle.vert.spv",&vz); if(!vc)return 1;
    ...
    uint32_t fv = 0;
    { const char*e=getenv("IND_FIRSTVERTEX"); if(e&&e[0]) fv=(uint32_t)atoi(e); }
    if(use_indirect){
        printf("calling vkCmdDrawIndirect(drawCount=1, stride=%zu)\n",
               sizeof(VkDrawIndirectCommand));
        CDrawInd(cmd,ibuf,0,1,sizeof(VkDrawIndirectCommand));
    } else {
        printf("calling vkCmdDraw(3,1,%u,0)\n",fv);
        CDraw(cmd,3,1,fv,0);
    }
    CER(cmd);
    CHECK(ECB(cmd),"end");
    ...
    for(int y=0;y<DIMS;y++){uint8_t*row=base+y*sl.rowPitch;
        for(int x=0;x<DIMS;x++){uint8_t*p=row+x*4; if(p[0]||p[1]||p[2]) nb++; }}
    ...
    if(use_indirect){
        ok = (nb == 0);
        printf("prediction was : 0 non-black (empty stub)\n");
        printf("verdict        : %s\n", ok
            ? "PREDICTION HELD - vkCmdDrawIndirect is a no-op on v9"
            : "PREDICTION FAILED - something was drawn, revisit the survey");
    } else {
        if(fv==0){
            ok = (nb == 512 && ce[0]==255 && ce[1]==0 && ce[2]==0);
            printf("prediction was : 512 non-black, centre 255,0,0\n");
            printf("verdict        : %s\n", ok ? "control OK" : "CONTROL BROKEN");
        } else {
            ok = 1;
            printf("firstVertex=%u probe, reporting only:\n",fv);
            printf("  512 non-black  -> firstVertex IGNORED (still triangle A)\n");
            printf("  253 non-black  -> firstVertex HONOURED (triangle B)\n");
        }
    }
    const char*pp=getenv("IND_PPM");
    if(pp&&pp[0]){
        FILE*f=fopen(pp,"wb");
        if(f){ fprintf(f,"P6\n%d %d\n255\n",DIMS,DIMS);
            for(int y=0;y<DIMS;y++){uint8_t*row=base+y*sl.rowPitch;
    ...
    UM(dev,imgm);
    printf("FINGERPRINT %s n=%u c=%u,%u,%u ok=%d\n",cs,nb,ce[0],ce[1],ce[2],ok);
    free(vc); free(fc);
    return ok?0:1;
}
