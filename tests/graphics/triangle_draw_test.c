/* triangle_draw_test.c
 *
 * Test CmdDraw v9 beneran: render 1 triangle fullscreen (warna merah solid)
 * ke image kecil (4x4), copy hasilnya ke buffer host-visible, baca pixel-nya.
 * Kalau CmdDraw v9 (Malloc Vertex Job + Fragment Job) beneran jalan bener,
 * pixel-nya harus merah (0xFF, 0x00, 0x00, 0xFF). Kalau nol semua atau warna
 * clear (hitam), berarti draw call nggak beneran nge-render apa-apa.
 *
 * Pola dlopen/entrypoint sama kayak compute_test.c dan
 * begin_end_rendering_test.c yang sudah kebukti jalan.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    PFN_vkCmdCopyImageToBuffer CmdCopyImageToBuffer = DPROC(CmdCopyImageToBuffer);
    ...
    CHECK(CreateBuffer(device, &buf_info, NULL, &readback_buf), "vkCreateBuffer(readback)");

    VkMemoryRequirements buf_mreq;
    GetBufferMemoryRequirements(device, readback_buf, &buf_mreq);
    uint32_t buf_mem_type = find_memory_type(&mem_props, buf_mreq.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    VkMemoryAllocateInfo buf_alloc = {
        .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .allocationSize = buf_mreq.size,
        .memoryTypeIndex = buf_mem_type,
    };
    VkDeviceMemory buf_mem;
    CHECK(AllocateMemory(device, &buf_alloc, NULL, &buf_mem), "vkAllocateMemory(buffer)");
    CHECK(BindBufferMemory(device, readback_buf, buf_mem, 0), "vkBindBufferMemory");
    ...
    /* --- Pipeline layout (kosong, shader nggak butuh descriptor) --- */
    ...
    /* PATCH diagnostic: isi sentinel 0xAA ke readback buffer SEBELUM submit,
     * biar bisa dibedain "GPU beneran nulis 0" vs "GPU nggak nyentuh buffer
     * ini sama sekali" (device-local fresh alloc bisa aja emang udah nol). */
    {
        void *sentinel_map;
        CHECK(MapMemory(device, buf_mem, 0, buf_size, 0, &sentinel_map), "vkMapMemory(sentinel-fill)");
        memset(sentinel_map, 0xAA, buf_size);
        UnmapMemory(device, buf_mem);
        printf("sentinel 0xAA ditulis ke readback buffer sebelum submit\n"); fflush(stdout);
    }
    ...
    CmdCopyImageToBuffer(cmdbuf, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    void *mapped;
    CHECK(MapMemory(device, buf_mem, 0, buf_size, 0, &mapped), "vkMapMemory(readback)");
    uint8_t *pixels = (uint8_t*)mapped;

    printf("\n--- hasil pixel (RGBA per pixel, harus 255 0 0 255 kalau triangle merah nge-render) ---\n");
    ...
    if (p0[0] == 255 && p0[1] == 0 && p0[2] == 0 && p0[3] == 255) {
        ...
        printf("\nFAILED: pixel (0,0) masih warna clear (hitam) -- triangle nggak ke-render.\n");
    } else {
        printf("\nUNEXPECTED: pixel (0,0) bukan merah atau hitam -- ada yang aneh, cek dump di atas.\n");
    }
