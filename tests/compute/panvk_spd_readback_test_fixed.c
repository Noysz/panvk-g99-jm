...
int main(void) {
    ...
    /* FIX: perlu buat invalidate cache CPU sebelum baca hasil tulisan GPU,
     * kalau memory type yang kepilih ternyata HOST_VISIBLE tapi BUKAN
     * HOST_COHERENT. Tanpa ini, host bisa baca data basi/sentinel walau GPU
     * udah beneran nulis. */
    PFN_vkInvalidateMappedMemoryRanges InvalidateMappedMemoryRanges = DPROC(device, vkInvalidateMappedMemoryRanges);
    ...
    /* FIX: sebelumnya `&` dengan OR-mask (VISIBLE|COHERENT) itu lolos kalau
     * SALAH SATU flag ada. Sekarang wajib DUA-DUANYA, biar dijamin host
     * lihat data GPU tanpa perlu invalidate manual -- tapi kita tetep pasang
     * invalidate di bawah buat jaga-jaga kalau ternyata satu-satunya
     * memory type yang tersedia bukan coherent. */
    uint32_t host_visible_type = UINT32_MAX;
    ...
    if (host_visible_type == UINT32_MAX) {
        /* fallback: minimal HOST_VISIBLE, invalidate WAJIB dipanggil nanti */
        for (uint32_t i = 0; i < mem_props.memoryTypeCount; i++) {
        ...
        printf("WARNING: no HOST_COHERENT memory type found, using HOST_VISIBLE-only type %u -- relying on explicit invalidate\n", host_visible_type);
    }
    printf("memory type dipilih: %u, propertyFlags=0x%x (coherent=%s)\n",
           host_visible_type, mem_props.memoryTypes[host_visible_type].propertyFlags,
           (mem_props.memoryTypes[host_visible_type].propertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) ? "YES" : "NO");
    ...
    /* FIX: invalidate cache CPU secara eksplisit sebelum baca -- wajib
     * dilakukan kalau memory type-nya HOST_VISIBLE tapi bukan
     * HOST_COHERENT. Aman dipanggil juga kalau memang sudah coherent
     * (no-op menurut spec). */
    if (InvalidateMappedMemoryRanges) {
        ...
        VkResult inv_result = InvalidateMappedMemoryRanges(device, 1, &range);
        printf("vkInvalidateMappedMemoryRanges result=%d\n", inv_result);
    } else {
        printf("WARNING: vkInvalidateMappedMemoryRanges proc addr NULL, skip invalidate\n");
    }

    /* --- Hasil paling penting: baca posisi clip-space langsung dari SSBO ---
     * FIX: print HEX MENTAH dulu, bukan cuma %.4f -- supaya "-0.0000" yang
     * muncul di print float tidak disalahartikan sebagai literal negative-
     * zero bit pattern (0x80000000) padahal bisa jadi cuma pembulatan dari
     * angka negatif sangat kecil / noise. Hex mentah tidak bisa berbohong. */
    printf("\n--- posisi vertex dari SSBO (skip tiler/rasterizer sepenuhnya) ---\n");
    uint32_t *raw = (uint32_t *)dbg_mapped;
    printf("--- hex mentah (12 word = 3x vec4) ---\n");
    for (int i = 0; i < 12; i++) {
        printf("word[%2d] = 0x%08x%s\n", i, raw[i],
               (raw[i] == 0xAAAAAAAAu) ? "  <-- MASIH SENTINEL, GPU TIDAK NULIS"
               : (raw[i] == 0x00000000u) ? "  <-- ZERO"
               : (raw[i] == 0x80000000u) ? "  <-- LITERAL NEGATIVE ZERO"
               : "");
    }

    float *p = (float *)dbg_mapped;
    printf("--- interpretasi float ---\n");
    for (int i = 0; i < 3; i++) {
        printf("vertex[%d] clip pos = (%.6f, %.6f, %.6f, %.6f)\n",
               i, p[i*4+0], p[i*4+1], p[i*4+2], p[i*4+3]);
    }
    printf("expected: vertex[0]=(0,-0.5,0,1) vertex[1]=(0.5,0.5,0,1) vertex[2]=(-0.5,0.5,0,1)\n");
    printf("(cek dulu HEX MENTAH di atas sebelum simpulkan apapun dari baris float)\n");
